...
function renderTable(){
  ...
  filtered.slice(start,Math.min(start+PAGE_SIZE,filtered.length)).forEach((row,i)=>{
    ...
    tr.innerHTML=`
      ...
      <td class="name-cell">${hasTitledPlayer?`<span class="titled-dot-wrap"><span class="titled-orbit-dot"></span></span>`:""}${row.name||'<span style="color:var(--muted);font-style:italic;">—</span>'}</td>
      <td>${row.birthYear||"—"}</td>
      <td>${row.gender||"—"}</td>
  ...
  (function injectRankingButtons(){
    ...
    if(!document.getElementById("rankingNavStyle")){
      ...
      style.textContent=`
        ...
        /* ── Titled player row ── */
        tr.titled-row{
          background:linear-gradient(90deg,#fffde7 0%,#fff9c4 60%,#fff8e1 100%) !important;
          position:relative;
        }
        tr.titled-row td{border-top:1.5px solid #FFD600;border-bottom:1.5px solid #FFD600;}
        tr.titled-row td:first-child{border-left:3px solid #FFD600;}
        tr.titled-row td:last-child {border-right:3px solid #FFD600;}
        ...
        .titled-orbit-dot{
          ...
          background:#FFD600;box-shadow:0 0 5px 2px rgba(255,214,0,.6);
          top:50%;left:50%;
          transform-origin:-6px 0;
          animation:orbitDot 1.8s linear infinite;
        }
        @keyframes orbitDot{
        ...
        /* Rated Player dropdown active highlight */
        select.active-filter{border-color:#0D47A1;background:#E3F2FD;color:#0D47A1;font-weight:700;}
      `;
