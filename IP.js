...
function applyFilter(){
  ...
  if(sortMode==="latest"){
    /* Latest first — parse dateOfEntry DD/MM/YY + timeOfEntry HH:MM:SS */
    data.sort((a,b)=>{
      const parseEntry=r=>{
        try{
          const [dd,mm,yy]=(r.dateOfEntry||"00/00/00").split("/");
          const dt=`20${yy}-${mm}-${dd}T${(r.timeOfEntry||"00:00:00")}`;
