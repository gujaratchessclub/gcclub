...
window.shareProfile=function(docId){
  ...
  /* Unlock state — inherited from current profile session */
  const unlocked=(currentProfile&&(currentProfile._adminOpen||currentProfile._ipOpen))||false;

  /* Canvas: portrait card 480x980 — taller to fit extra fields */
  const W=480, H=980;
  ...
  /* Helper: wrap text, returns final y */
  ...
  /* Helper: draw one info row (label + value), returns next y */
  function drawInfoRow(label,val,x,y,colW){
    ctx.fillStyle="#90A4AE"; ctx.font="10px Arial"; ctx.textAlign="left";
    ctx.fillText(label.toUpperCase(),x,y);
    ctx.fillStyle="#263238"; ctx.font="bold 13px Arial";
    /* truncate value if too wide */
    let display=val;
    while(display.length>1&&ctx.measureText(display).width>colW-6) display=display.slice(0,-1)+"…";
    ctx.fillText(display,x,y+16);
    return y+34;
  }
  ...
  function drawCard(photoImg){
    ...
    const ratingDefs=[
      {l:"STD Rating",  v:row.stdRating||"—",  bg:"#EEF4FF",col:"#0D47A1",brd:"#BBDEFB"},
    ...
    let curY=gY;
    drawInfoRow("Birth Year", row.birthYear||"—",        ML,        curY, colW);
    drawInfoRow("Gender",     row.gender||"—",            ML+colW,  curY, colW);
    curY+=34;

    /* Row 2: District | Status */
    drawInfoRow("District",   (row.district||"—").toUpperCase(), ML,        curY, colW);
    drawInfoRow("Status",     row.status||"—",                   ML+colW,  curY, colW);
    curY+=34;

    /* Thin divider */
    curY+=4;
    ctx.strokeStyle="#EEF2FF"; ctx.lineWidth=1;
    ctx.beginPath(); ctx.moveTo(ML,curY); ctx.lineTo(W-ML,curY); ctx.stroke();
    curY+=10;

    /* Row 3: Title | Arbiter Title */
    drawInfoRow("Title",         row.title||"—",         ML,        curY, colW);
    drawInfoRow("Arbiter Title", row.arbiterTitle||"—",  ML+colW,  curY, colW);
    curY+=34;

    /* Row 4: Trainer Title | Arena Title */
    drawInfoRow("Trainer Title", row.trainerTitle||"—",  ML,        curY, colW);
    drawInfoRow("Arena Title",   row.arenaTitle||"—",    ML+colW,  curY, colW);
    curY+=34;
    ...
    /* Row 5: Date of Birth | Contact No  — masked unless unlocked */
    const dobVal  = unlocked ? (row.dobRaw||row.dob||"—") : (row.dobRaw||row.dob ? "••/••/••••" : "—");
    const contVal = unlocked ? (row.contactNo||"—") : maskLast(row.contactNo,3);
    drawInfoRow("Date of Birth", dobVal,  ML,        curY, colW);
    drawInfoRow("Contact No",    contVal, ML+colW,  curY, colW);
    curY+=34;

    /* Row 6: Email ID — full width, masked unless unlocked */
    const emailVal = unlocked ? (row.email||"—") : maskEmail(row.email);
    ctx.fillStyle="#90A4AE"; ctx.font="10px Arial"; ctx.textAlign="left";
    ctx.fillText("EMAIL ID",ML,curY);
    ctx.fillStyle="#263238"; ctx.font="bold 13px Arial";
    let emailDisplay=emailVal;
    while(emailDisplay.length>1&&ctx.measureText(emailDisplay).width>W-ML*2-6) emailDisplay=emailDisplay.slice(0,-1)+"…";
    ctx.fillText(emailDisplay,ML,curY+16);
    curY+=34;

    /* ── Lock notice when not unlocked ── */
    if(!unlocked){
      curY+=6;
      ctx.fillStyle="#FFF8E1";
      rr(ML,curY,W-ML*2,28,6); ctx.fill();
      ctx.strokeStyle="#FFE082"; ctx.lineWidth=1; rr(ML,curY,W-ML*2,28,6); ctx.stroke();
      ctx.fillStyle="#E65100"; ctx.font="bold 11px Arial"; ctx.textAlign="center";
      ctx.fillText("🔐 Login as Admin or IP to reveal private details",W/2,curY+18);
      curY+=36;
    }
