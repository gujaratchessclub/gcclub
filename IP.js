  ...
  function drawCard(photoImg){
    ...
    if(!ctx.roundRect){
      ctx.roundRect=function(x,y,w,h,r){
        const rad=typeof r==="number"?r:(r&&r.upperLeft)||8;
        ctx.moveTo(x+rad,y);
    ...
    grad.addColorStop(0,"#0D47A1"); grad.addColorStop(1,"#1565C0");
    ...
    ctx.beginPath();
    ctx.roundRect(20,20,W-40,56,{upperLeft:16,upperRight:16,lowerRight:0,lowerLeft:0});
    ctx.fill();
    ctx.fillStyle="#fff";
    ctx.font="bold 15px Arial";
    ctx.textAlign="center";
    ctx.fillText("Gujarat Chess Club — IP Profile",W/2,55);

    /* Photo */
    const PW=130,PH=165,px=(W-PW)/2,py=92;
    ctx.save();
    ctx.beginPath();
    ctx.roundRect(px,py,PW,PH,10);
    ctx.clip();
    if(photoImg){ ctx.drawImage(photoImg,px,py,PW,PH); }
    else{
      ctx.fillStyle="#E3F2FD"; ctx.fillRect(px,py,PW,PH);
      ctx.fillStyle="#90CAF9"; ctx.font="56px Arial";
      ctx.textAlign="center"; ctx.fillText("?",W/2,py+PH/2+18);
    }
    ctx.restore();

    /* Name */
    ctx.fillStyle="#0D47A1"; ctx.font="bold 20px Arial";
    ctx.textAlign="center";
    ctx.fillText(row.name||"—",W/2,282);

    /* FIDE ID */
    if(row.fideId){ ctx.fillStyle="#1565C0"; ctx.font="13px Arial"; ctx.fillText("FIDE ID: "+row.fideId,W/2,302); }
    /* AICF ID */
    if(row.aicfId){ ctx.fillStyle="#455A64"; ctx.font="12px Arial"; ctx.fillText("AICF ID: "+row.aicfId,W/2,320); }

    /* Rating cards */
    const ratings=[
      {l:"Std",v:row.stdRating||"—",bg:"#E3F2FD",col:"#0D47A1"},
      {l:"Rapid",v:row.rapidRating||"—",bg:"#E8F5E9",col:"#2E7D32"},
      {l:"Blitz",v:row.blitzRating||"—",bg:"#FFEBEE",col:"#B71C1C"},
    ];
    const rw=110,rh=52,gap=10,rTotalW=rw*3+gap*2,rX=(W-rTotalW)/2,rY=336;
    ratings.forEach((r,i)=>{
      const rx=rX+i*(rw+gap);
      ctx.fillStyle=r.bg;
      ctx.beginPath(); ctx.roundRect(rx,rY,rw,rh,8); ctx.fill();
      ctx.fillStyle=r.col; ctx.font="bold 17px Arial"; ctx.textAlign="center";
      ctx.fillText(r.v,rx+rw/2,rY+26);
      ctx.font="10px Arial"; ctx.fillText(r.l,rx+rw/2,rY+44);
    });
    ...
    const infos=[
      ...
      row.district?"District: "+(row.district||"").toUpperCase():null,
      row.birthYear?"Birth Year: "+row.birthYear:null,
      row.status?"Status: "+row.status:null,
    ].filter(Boolean);
    ctx.textAlign="center"; ctx.font="12px Arial"; ctx.fillStyle="#455A64";
    infos.forEach((info,i)=>{ ctx.fillText(info,W/2,404+i*19); });

    /* IP ID */
    ctx.fillStyle="#90A4AE"; ctx.font="11px Arial"; ctx.textAlign="center";
    ctx.fillText("IP ID: "+(row.ipId||"—"),W/2,H-46);

    /* Footer */
    ctx.fillStyle="#0D47A1";
    ctx.beginPath();
    ctx.roundRect(20,H-38,W-40,18,{upperLeft:0,upperRight:0,lowerLeft:16,lowerRight:16});
    ctx.fill();
    ctx.fillStyle="#fff"; ctx.font="10px Arial"; ctx.textAlign="center";
    ctx.fillText("Gujarat Chess Club",W/2,H-25);

    /* Share or download */
    canvas.toBlob(blob=>{
      if(!blob){showToast("❌ Could not generate image.");return;}
      const file=new File([blob],"profile_"+(row.name||row.ipId||"player").replace(/\s+/g,"_")+".jpg",{type:"image/jpeg"});
