...
let activeFilters = {
  ...
  arenaTitled:false,  ratedFilter:"",    noName:false, dupEntry:false, nonFideId:false
};
...
/* A player is "rated" if any of the 3 ratings has a non-empty, non-zero value */
function isRated(r){
  const check=v=>{ const s=String(v||"").trim(); return s!==""&&s!=="0"; };
  return check(r.stdRating)||check(r.rapidRating)||check(r.blitzRating);
}

/* Convert any time string to 12-hour AM/PM for display.
   Handles: "14:30:22" → "2:30:22 PM"  |  "9:05 AM" → unchanged */
function formatTimeAmPm(raw){
  ...
  // Parse 24-hr  HH:MM  or  HH:MM:SS
  const m=t.match(/^(\d{1,2}):(\d{2})(?::(\d{2}))?$/);
  if(!m) return t; // unrecognised format — show as-is
...
function showSkeleton(){
  ...
  for(let i=0;i<8;i++){
    rows+=`<tr class="sk-row">
      <td><div class="sk-cell" style="width:42px;height:52px;border-radius:5px;"></div></td>
      <td><div class="sk-cell" style="width:26px;"></div></td>
      <td><div class="sk-cell" style="width:72px;"></div></td>
      <td><div class="sk-cell" style="width:36px;height:18px;border-radius:10px;"></div></td>
      <td><div class="sk-cell" style="width:120px;"></div></td>
      <td><div class="sk-cell" style="width:42px;"></div></td>
      <td><div class="sk-cell" style="width:42px;"></div></td>
      <td><div class="sk-cell" style="width:72px;height:18px;border-radius:10px;"></div></td>
      <td><div class="sk-cell" style="width:64px;"></div></td>
      <td><div class="sk-cell" style="width:58px;height:26px;border-radius:20px;"></div></td>
    </tr>`;
  }
...
async function loadData(){
  ...
  try{
    ...
    // Render first page immediately for instant feel
    applyFilter();
    // Heavier processing deferred so first paint isn't blocked
    setTimeout(()=>{ detectDups(); populateDistricts(); applyFilter(); },0);
  }catch(e){
...
window.toggleFilterBtn=function(key){
  ...
  const ids={titledPlayer:"btnTitledPlayer",fideArbiter:"btnFideArbiter",fideTrainer:"btnFideTrainer",
    arenaTitled:"btnArenaTitled",noName:"btnNoName",dupEntry:"btnDupEntry",nonFideId:"btnNonFideId"};
  document.getElementById(ids[key]).classList.toggle("active",activeFilters[key]);
  currentPage=1; applyFilter();
};

window.setRatedFilter=function(val){
  activeFilters.ratedFilter=val;
  /* update dropdown visual highlight */
  const sel=document.getElementById("fRatedFilter");
...
function applyFilter(){
  ...
  /* Rated/Unrated dropdown — "rated" = any of 3 ratings has a value; "unrated" = all 3 blank/0 */
  if(activeFilters.ratedFilter==="rated"){
    data=data.filter(r=>isRated(r));
  } else if(activeFilters.ratedFilter==="unrated"){
    data=data.filter(r=>!isRated(r));
  }
...
window.clearFilters=function(){
  ...
  /* Reset rated dropdown */
  const rSel=document.getElementById("fRatedFilter");
  if(rSel){rSel.value="";rSel.classList.remove("active-filter");}
  Object.keys(activeFilters).forEach(k=>{
    if(k==="ratedFilter") activeFilters[k]="";
    else activeFilters[k]=false;
  });
  ["btnTitledPlayer","btnFideArbiter","btnFideTrainer","btnArenaTitled",
   "btnNoName","btnDupEntry","btnNonFideId"].forEach(id=>{
...
function renderTable(){
  ...
  tbody.innerHTML="";
  if(!filtered.length){
    tbody.innerHTML=`<tr><td colspan="${adminMode?14:11}" style="text-align:center;padding:48px;color:var(--muted);">No records found.</td></tr>`;
    return;
  }
  ...
  filtered.slice(start,Math.min(start+PAGE_SIZE,filtered.length)).forEach((row,i)=>{
    ...
    /* Titled player — gold highlight + animated border */
    const hasTitledPlayer=!!(row.title||"").trim();
    if(hasTitledPlayer) tr.classList.add("titled-row");
    ...
    tr.innerHTML=`
      ...
      ${adminMode?`<td class="entry-date-cell">${row.dateOfEntry||"—"}</td><td class="entry-time-cell">${formatTimeAmPm(row.timeOfEntry||"")}</td><td class="entry-entby-cell">${row.enteredBy||"—"}</td>`:""}
    `;
...
function buildRoles(r){
  const p=[];
  if(isRated(r))                             p.push(`<span class="role-tag rated">Rated Player</span>`);
...
/* ════════════════════════════════════
   SHARE PROFILE — canvas JPG image
════════════════════════════════════ */
window.shareProfile=function(docId){
  const row=allData.find(r=>r._docId===docId);
  if(!row){showToast("❌ Record not found.");return;}

  /* Canvas: portrait card 480x700 */
  const W=480, H=700;
  const canvas=document.createElement("canvas");
  canvas.width=W; canvas.height=H;
  const ctx=canvas.getContext("2d");

  /* Helper: draw rounded rect (polyfill) */
  ...
  function drawCard(photoImg){
    ...
    /* Top blue header — slim */
    ctx.fillStyle="#0D47A1";
    ctx.fillRect(0,0,W,54);
    ctx.fillStyle="#FFFFFF";
    ctx.font="bold 14px Arial";
    ctx.textAlign="center";
    ctx.fillText("Gujarat Chess Club",W/2,22);
    ctx.font="12px Arial";
    ctx.fillStyle="rgba(255,255,255,0.85)";
    ctx.fillText("IP Profile Card",W/2,42);

    /* Photo — left side */
    const PX=28, PY=70, PW=120, PH=150;
    ctx.save();
    ...
    if(photoImg){
      ...
      ctx.fillStyle="#9AAAC8"; ctx.font="48px Arial"; ctx.textAlign="center";
      ctx.fillText("👤",PX+PW/2,PY+PH/2+16);
    }
    ctx.restore();
    /* Photo border */
    ...
    /* Name — right of photo */
    const TX=PX+PW+16, TMaxW=W-TX-16;
    ctx.fillStyle="#0D47A1"; ctx.font="bold 17px Arial"; ctx.textAlign="left";
    const nameY=wrapText(row.name||"—",TX,92,TMaxW,22);

    /* FIDE ID */
    let infoY=nameY+14;
    if(row.fideId){
      ctx.fillStyle="#1565C0"; ctx.font="bold 12px Arial";
      ctx.fillText("FIDE: "+row.fideId,TX,infoY); infoY+=18;
    }
    /* AICF ID */
    if(row.aicfId){
      ctx.fillStyle="#455A64"; ctx.font="12px Arial";
      ctx.fillText("AICF: "+row.aicfId,TX,infoY); infoY+=18;
    }
    /* IP ID */
    ctx.fillStyle="#78909C"; ctx.font="11px Arial";
    ctx.fillText("IP ID: "+(row.ipId||"—"),TX,infoY); infoY+=18;

    /* Title badge */
    if(row.title){
      ctx.fillStyle="#E8F5E9";
      rr(TX,infoY-14,ctx.measureText(row.title).width+16,20,10);
      ctx.fill();
      ctx.fillStyle="#2E7D32"; ctx.font="bold 11px Arial";
      ctx.fillText(row.title,TX+8,infoY);
      infoY+=24;
    }

    /* Divider */
    const divY=Math.max(PY+PH,infoY)+12;
    ctx.strokeStyle="#E3F2FD"; ctx.lineWidth=1.5;
    ctx.beginPath(); ctx.moveTo(20,divY); ctx.lineTo(W-20,divY); ctx.stroke();

    /* Rating cards — 3 in a row */
    const rY=divY+10;
    const rw=(W-56)/3, rh=60, rgap=8;
    const ratingDefs=[
      {l:"Standard Rating",v:row.stdRating||"—",bg:"#EEF4FF",col:"#0D47A1",brd:"#BBDEFB"},
      {l:"Rapid Rating",v:row.rapidRating||"—",bg:"#F0FFF4",col:"#1B5E20",brd:"#A5D6A7"},
      {l:"Blitz Rating",v:row.blitzRating||"—",bg:"#FFF5F5",col:"#B71C1C",brd:"#EF9A9A"},
    ];
    ratingDefs.forEach((r,i)=>{
      const rx=20+i*(rw+rgap);
      ctx.fillStyle=r.bg; rr(rx,rY,rw,rh,8); ctx.fill();
      ctx.strokeStyle=r.brd; ctx.lineWidth=1.2; rr(rx,rY,rw,rh,8); ctx.stroke();
      ctx.fillStyle=r.col; ctx.font="bold 20px Arial"; ctx.textAlign="center";
      ctx.fillText(r.v,rx+rw/2,rY+32);
      ctx.fillStyle="#78909C"; ctx.font="10px Arial";
      ctx.fillText(r.l,rx+rw/2,rY+50);
    });

    /* Info grid below ratings */
    const gY=rY+rh+16;
    const info2col=[
    ...
    ctx.textAlign="left";
    const colW=(W-40)/2;
    info2col.forEach(([label,val],i)=>{
      const cx=20+(i%2)*colW, cy=gY+Math.floor(i/2)*46;
      ctx.fillStyle="#90A4AE"; ctx.font="10px Arial";
      ctx.fillText(label.toUpperCase(),cx,cy+12);
      ctx.fillStyle="#263238"; ctx.font="bold 13px Arial";
      ctx.fillText(val,cx,cy+28);
    });
    ...
    ctx.fillText("Gujarat Chess Club — IP Database",W/2,H-14);

    /* Share or download */
    canvas.toBlob(blob=>{
      if(!blob){showToast("❌ Could not generate image.");return;}
      const fname="profile_"+(row.name||row.ipId||"player").replace(/\s+/g,"_")+".jpg";
      const file=new File([blob],fname,{type:"image/jpeg"});
      if(navigator.share&&navigator.canShare&&navigator.canShare({files:[file]})){
        navigator.share({files:[file],title:"IP Profile — "+(row.name||""),text:"Gujarat Chess Club"}).catch(()=>{});
      } else {
        const url=URL.createObjectURL(blob);
        const a=document.createElement("a");
        a.href=url; a.download=fname; a.click();
        setTimeout(()=>URL.revokeObjectURL(url),1000);
        showToast("📥 Profile image downloaded!");
      }
    },"image/jpeg",0.92);
  }
...
/* ════════════════════════════════════
   SHARE DETAILS — copy to clipboard
════════════════════════════════════ */
window.shareDetails=function(docId){
  const row=allData.find(r=>r._docId===docId);
  if(!row){showToast("❌ Record not found.");return;}
  const isAdmin=currentProfile&&currentProfile._adminOpen;
  const isIp=currentProfile&&currentProfile._ipOpen;
  const unlocked=isAdmin||isIp;

  const lines=[
    "Gujarat Chess Club — IP Profile",
    "─────────────────────────────────",
    "Name          : "+(row.name||"—"),
    "IP ID         : "+(row.ipId||"—"),
    "FIDE ID       : "+(row.fideId||"—"),
    ...
    "Status        : "+(row.status||"—"),
    "Title         : "+(row.title||"—"),
    "Arbiter Title : "+(row.arbiterTitle||"—"),
    "Trainer Title : "+(row.trainerTitle||"—"),
    "Arena Title   : "+(row.arenaTitle||"—"),
    "Std Rating    : "+(row.stdRating||"—"),
    "Rapid Rating  : "+(row.rapidRating||"—"),
    "Blitz Rating  : "+(row.blitzRating||"—"),
    "Date of Birth : "+(unlocked?(row.dobRaw||row.dob||"—"):"••/••/••••"),
    "Contact No    : "+(unlocked?(row.contactNo||"—"):maskLast(row.contactNo,3)),
    "Email         : "+(unlocked?(row.email||"—"):maskEmail(row.email)),
    "Lichess ID    : "+(unlocked?(row.lichessId||"—"):maskLast(row.lichessId,2)),
    "Chess.com ID  : "+(unlocked?(row.chesscomId||"—"):maskLast(row.chesscomId,2)),
    "─────────────────────────────────",
    "Gujarat Chess Club",
  ].join("\n");
  ...
  if(navigator.clipboard&&navigator.clipboard.writeText){
    navigator.clipboard.writeText(lines)
      ...
      .catch(()=>clipFb(lines,"📋 Details copied!"));
  } else {
    clipFb(lines,"📋 Details copied!");
  }
};
...
function renderProfile(row, adminOpen, ipOpen){
  ...
  /* share buttons — only after admin or IP login */
  ...
  document.getElementById("profileContent").innerHTML=`
    ...
    <div class="rating-highlight-row">
      ...
      <div class="rating-card rap-card">
        <div class="rating-card-label">🚀 Rapid Rating</div>
...
/* ════════════════════════════════════════
   FORGOT / CREATE PIN
════════════════════════════════════════ */
let _fpDocId=null; // holds the Firestore doc ID of the looked-up IP

window.openForgotPinModal=function(){
  _fpDocId=null;
  // Reset all steps
  document.getElementById("fpStep1").style.display="";
  document.getElementById("fpStep2Create").style.display="none";
  document.getElementById("fpStep2Change").style.display="none";
  document.getElementById("fpIpIdInput").value="";
  document.getElementById("fpStep1Err").textContent="";
  document.getElementById("forgotPinModal").classList.add("active");
  setTimeout(()=>document.getElementById("fpIpIdInput").focus(),120);
};
...
window.fpBackToStep1=function(){
  ...
  document.getElementById("fpStep1Err").textContent="";
  _fpDocId=null;
  setTimeout(()=>document.getElementById("fpIpIdInput").focus(),80);
};

window.fpLookupId=async function(){
  const ipId=(document.getElementById("fpIpIdInput").value||"").trim().toUpperCase();
  const errEl=document.getElementById("fpStep1Err");
  if(!ipId){ errEl.textContent="❌ Please enter your IP ID."; return; }

  errEl.textContent="⏳ Looking up…";

  // Search in already-loaded data first (instant)
  let found=allData.find(r=>(r.ipId||"").trim().toUpperCase()===ipId);

  // If allData not yet loaded, query Firestore directly
  if(!found){
    try{
      const q=query(collection(db,DB_NAME),where("ipId","==",ipId));
      const snap=await getDocs(q);
      if(!snap.empty){
        const d=snap.docs[0];
        found={_docId:d.id,...d.data()};
        // Merge into allData cache if missing
        if(!allData.find(r=>r._docId===found._docId)) allData.push(found);
      }
    }catch(e){
  ...
  if(!found){
    errEl.textContent="❌ IP ID not found. Check and try again.";
    return;
  }

  _fpDocId=found._docId;
  errEl.textContent="";
  document.getElementById("fpStep1").style.display="none";

  const storedPin=String(found.ipPin||"").trim();
  if(storedPin==="147258"){
    // Default PIN — guide user to create their own
    document.getElementById("fpNewPin").value="";
    document.getElementById("fpConfirmPin").value="";
    document.getElementById("fpCreateErr").textContent="";
    document.getElementById("fpStep2Create").style.display="";
    setTimeout(()=>document.getElementById("fpNewPin").focus(),80);
  } else {
    // Has a custom PIN — let them change it
    document.getElementById("fpCurrentPin").value="";
    document.getElementById("fpNewPinChange").value="";
    document.getElementById("fpConfirmPinChange").value="";
    document.getElementById("fpChangeErr").textContent="";
    document.getElementById("fpStep2Change").style.display="";
    setTimeout(()=>document.getElementById("fpCurrentPin").focus(),80);
  }
};

window.fpSubmitCreate=async function(){
  const p1=(document.getElementById("fpNewPin").value||"").trim();
  const p2=(document.getElementById("fpConfirmPin").value||"").trim();
  const errEl=document.getElementById("fpCreateErr");
  if(!p1||p1.length<4){ errEl.textContent="❌ PIN must be at least 4 digits."; return; }
  if(!/^\d+$/.test(p1)){ errEl.textContent="❌ PIN must contain digits only."; return; }
  if(p1==="147258"){ errEl.textContent="❌ Cannot use this PIN. Please choose a different one."; return; }
  if(p1!==p2){ errEl.textContent="❌ PINs do not match. Try again."; return; }
  errEl.textContent="⏳ Saving…";
  try{
    await updateDoc(doc(db,DB_NAME,_fpDocId),{ipPin:p1});
    // Update local cache
    const idx=allData.findIndex(r=>r._docId===_fpDocId);
    if(idx>=0) allData[idx].ipPin=p1;
    closeModal("forgotPinModal");
    showToast("✅ IP PIN created successfully!");
  }catch(e){
    errEl.textContent="❌ Failed to save. Check connection.";
  }
};

window.fpSubmitChange=async function(){
  const cur =(document.getElementById("fpCurrentPin").value||"").trim();
  const p1  =(document.getElementById("fpNewPinChange").value||"").trim();
  const p2  =(document.getElementById("fpConfirmPinChange").value||"").trim();
  const errEl=document.getElementById("fpChangeErr");
  const rec=allData.find(r=>r._docId===_fpDocId);
  if(!rec){ errEl.textContent="❌ Record not found."; return; }
  if(String(rec.ipPin||"").trim()!==cur){ errEl.textContent="❌ Current PIN is incorrect."; return; }
  if(!p1||p1.length<4){ errEl.textContent="❌ New PIN must be at least 4 digits."; return; }
  if(!/^\d+$/.test(p1)){ errEl.textContent="❌ PIN must contain digits only."; return; }
  if(p1==="147258"){ errEl.textContent="❌ Cannot use this PIN. Please choose a different one."; return; }
  if(p1!==p2){ errEl.textContent="❌ New PINs do not match. Try again."; return; }
  errEl.textContent="⏳ Saving…";
  try{
    await updateDoc(doc(db,DB_NAME,_fpDocId),{ipPin:p1});
    const idx=allData.findIndex(r=>r._docId===_fpDocId);
    if(idx>=0) allData[idx].ipPin=p1;
    closeModal("forgotPinModal");
    showToast("✅ IP PIN updated successfully!");
  }catch(e){
    errEl.textContent="❌ Failed to save. Check connection.";
  }
};
...
function init(){
  ...
  /* ── Rankings nav buttons in header ── */
  (function injectRankingButtons(){
    /* Find a container in the header — try .header-actions, .header-right, or the header itself */
    const targets=["header-actions","header-right","header-nav","topbar-right"];
    let container=null;
    targets.forEach(cls=>{if(!container){const el=document.querySelector("."+cls);if(el)container=el;}});
    if(!container){
      /* fallback: append after the first <header> or <nav> element */
      container=document.querySelector("header")||document.querySelector("nav")||document.body;
    }
    const btnBar=document.createElement("div");
    btnBar.id="rankingsBtnBar";
    btnBar.style.cssText="display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin:6px 0;";
    btnBar.innerHTML=`
      <a class="ranking-nav-btn std-nav-btn"  href="https://gujaratchessclub.github.io/gcclub/stdrankings.html"  target="_blank" rel="noopener">🏆 Standard Rankings</a>
      <a class="ranking-nav-btn rapid-nav-btn" href="https://gujaratchessclub.github.io/gcclub/rapidrankings.html" target="_blank" rel="noopener">⚡ Rapid Rankings</a>
      <a class="ranking-nav-btn blitz-nav-btn" href="https://gujaratchessclub.github.io/gcclub/blitzrankings.html" target="_blank" rel="noopener">🔥 Blitz Rankings</a>
    `;
    /* Inject ranking nav button styles if not already present */
    if(!document.getElementById("rankingNavStyle")){
      const style=document.createElement("style");
      style.id="rankingNavStyle";
      style.textContent=`
        .ranking-nav-btn{
          display:inline-flex;align-items:center;gap:5px;
          padding:7px 14px;border-radius:8px;font-size:13px;font-weight:700;
          text-decoration:none;color:#fff;letter-spacing:.3px;
          transition:opacity .18s,transform .15s;white-space:nowrap;
          box-shadow:0 2px 6px rgba(0,0,0,.18);
        }
        .ranking-nav-btn:hover{opacity:.88;transform:translateY(-1px);}
        .std-nav-btn  {background:linear-gradient(135deg,#1565C0,#1976D2);}
        .rapid-nav-btn{background:linear-gradient(135deg,#2E7D32,#388E3C);}
        .blitz-nav-btn{background:linear-gradient(135deg,#B71C1C,#D32F2F);}
      `;
  ...
  /* Forgot/Create PIN — keyboard Enter support */
  const fpId=document.getElementById("fpIpIdInput");
  if(fpId){
    fpId.addEventListener("input",function(){this.value=this.value.toUpperCase();});
    fpId.addEventListener("keydown",e=>{if(e.key==="Enter")window.fpLookupId();});
  }
  ["fpNewPin","fpConfirmPin","fpCurrentPin","fpNewPinChange","fpConfirmPinChange"].forEach(id=>{
    const el=document.getElementById(id); if(!el) return;
    el.addEventListener("input",function(){this.value=this.value.replace(/\D/g,"");});
    el.addEventListener("keydown",e=>{
      if(e.key!=="Enter") return;
      if(id==="fpConfirmPin")    window.fpSubmitCreate();
      else if(id==="fpConfirmPinChange") window.fpSubmitChange();
    });
  });
}
