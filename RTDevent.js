...
// ─────────────────────────────────────────────────────────────────────────────
// PORTRAIT SHARE CARD — pure white bg, exact popup mirror
// ─────────────────────────────────────────────────────────────────────────────
async function generatePortraitCard(ev, deepLink) {
  ...
  // 1. EVENT NAME (popup: 21px 800 #1A237E)
  y += 56;
  c.textAlign = "left";
  const evName = ev.eventName || "—";
  const nameSz = evName.length > 55 ? 38 : evName.length > 38 ? 44 : 50;
  c.font = `800 ${nameSz}px Arial, sans-serif`;
  c.fillStyle = "#1A237E";
  const nameLines = imgWrap(c, evName, TW, nameSz);
  nameLines.forEach(line => { c.fillText(line, PAD, y); y += nameSz + 12; });
  y += 4;

  // 2. ORGANIZER (popup: 13px 400 #78909C)
  ...
  // 3. LINKS
  y = imgSecTitle(c, "🔗 Links", PAD, y, TW);
  const lnks = ev.links || {};
  const linkDefs = [
    {t:"📄 Brochure",        u:lnks.brochure},
    {t:"♟ Chess Results",   u:lnks.chessResults},
    {t:"📍 Map Venue",       u:lnks.map},
    {t:"🎥 Live Games",      u:lnks.liveGames},
    {t:"🏆 Prize List",      u:lnks.prizeList},
    {t:"📝 Registration",    u:lnks.register},
    {t:"♟ Register via GCC",u:lnks.registerGcc},
    {t:"📸 Event Photos",    u:lnks.photographs},
  ];
  const CW2=(TW-16)/3, CH=54, CG=8;
  let cx=PAD;
  linkDefs.forEach((lnk,i)=>{
    if(i>0&&i%3===0){y+=CH+CG;cx=PAD;}
    const act=!!lnk.u;
    c.fillStyle=act?"#EEF4FF":"#F5F5F5";
    imgRound(c,cx,y,CW2,CH,14);c.fill();
    if(act){c.strokeStyle="#BBDEFB";c.lineWidth=2;imgRound(c,cx,y,CW2,CH,14);c.stroke();}
    c.font=`${act?"600":"400"} 22px Arial, sans-serif`;
    c.fillStyle=act?"#1A237E":"#BDBDBD";
    c.textAlign="left";
    let lt=lnk.t;
    const mw=CW2-20;
    while(c.measureText(lt).width>mw&&lt.length>3)lt=lt.slice(0,-1);
    if(lt!==lnk.t)lt+="…";
    c.fillText(lt,cx+10,y+35);
    cx+=CW2+CG;
  });
  y+=CH+24;c.textAlign="left";

  // 4. EVENT DETAILS
  y = imgSecTitle(c, "📋 Event Details", PAD, y, TW);
  const detRows=[
  ...
  let tsy=y;
  detRows.forEach(([l,v],i)=>{y=imgRow(c,l,v,PAD,y,TW,i,detRows.length);});
  c.strokeStyle="#BBDEFB";c.lineWidth=2;c.strokeRect(PAD,tsy,TW,y-tsy);
  y+=24;
  ...
  y=imgSecTitle(c,"💰 Entry Fees",PAD,y,TW);
  let fsy=y;
  fRows.forEach(([l,v],i)=>{y=imgRow(c,l,v,PAD,y,TW,i,fRows.length);});
  c.strokeStyle="#BBDEFB";c.lineWidth=2;c.strokeRect(PAD,fsy,TW,y-fsy);
  y+=24;

  // 6. TOURNAMENT DIRECTOR
  const td=ev.tournamentDirector||{};
  if(td.name){
    y=imgSecTitle(c,"🎯 Tournament Director",PAD,y,TW);
    let tdsy=y;
    y=imgRow(c,"Name",td.name+(td.fideId?`  (FIDE: ${td.fideId})`:""),PAD,y,TW,0,1);
    c.strokeStyle="#BBDEFB";c.lineWidth=2;c.strokeRect(PAD,tdsy,TW,y-tdsy);
    y+=24;
  }
  ...
  y=imgSecTitle(c,"⚖️ Arbiters",PAD,y,TW);
  let asy=y;
  aRows.forEach(([l,v],i)=>{y=imgRow(c,l,v,PAD,y,TW,i,aRows.length);});
  c.strokeStyle="#BBDEFB";c.lineWidth=2;c.strokeRect(PAD,asy,TW,y-asy);
  y+=24;
  ...
  c.fillStyle="#90A4AE";
  c.textAlign="center";
  c.fillText("Rated Events GJ  •  Gujarat Chess Club",W/2,y+28);
  y+=64;

  // Crop to content
  const fin=document.createElement("canvas");
  fin.width=W; fin.height=y;
  const fc=fin.getContext("2d");
  fc.fillStyle="#FFFFFF"; fc.fillRect(0,0,W,y);
  fc.drawImage(canvas,0,0,W,y,0,0,W,y);
  return fin.toDataURL("image/jpeg",0.97);
}
