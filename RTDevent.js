...
// ─────────────────────────────────────────────────────────────────────────────
// SHARE CARD — 1080×1350, NO links, HD, content vertically centered
// ─────────────────────────────────────────────────────────────────────────────

// First-pass: measure total content height at given scale, then center it
async function generatePortraitCard(ev, deepLink) {
  const OUT_W = 1080, OUT_H = 1350;
  const S = 3; // render at 3× for HD, downscale to OUT_W×OUT_H
  const W = OUT_W * S; // 3240
  const H = OUT_H * S; // 4050
  ...
  // ── Sizes (all in render pixels = CSS px × S) ──
  const PAD      = 44 * S;   // horizontal outer padding
  const TW       = W - PAD * 2;
  const LW_RATIO = 0.40;     // label column = 40% of table width
  const SEC_H    = 36 * S;   // section header bar height
  const ROW_H    = 44 * S;   // base row height (single-line value)
  const LABEL_SZ = 11 * S;   // label font size
  const VAL_SZ   = 14 * S;   // value font size
  const NAME_SZ  = (ev.eventName||"").length > 55 ? 22*S : (ev.eventName||"").length > 38 ? 26*S : 30*S;
  const ORG_SZ   = 14 * S;
  const FOOT_SZ  = 11 * S;
  const GAP      = 14 * S;   // gap between sections
  const DIV_GAP  = 16 * S;   // gap around divider

  // ── Measure pass (dry run to get total height) ──
  // We use an offscreen canvas just for text measurement
  ...
  let contentH = 0;
  contentH += nameH + 4*S;                   // title
  contentH += ORG_SZ + 16*S;                 // organizer
  contentH += DIV_GAP + 1 + DIV_GAP;        // divider line
  ...
  contentH += DIV_GAP + FOOT_SZ + 24*S;     // footer

  // ── Vertical centering: equal top/bottom padding ──
  ...
  function drawSection(title, rows) {
    // Header bar — solid blue like popup
    c.fillStyle = "#1565C0";
    ...
    rows.forEach(([label, value], i) => {
      ...
      // Label cell background — blue tint (same as popup td:first-child)
      c.fillStyle = "#EEF4FF";
      ...
      // Label text — uppercase, bold, blue
  ...
  c.fillStyle = "#90A4AE";
  c.textAlign = "center";
  c.fillText("Rated Events GJ  •  Gujarat Chess Club", W/2, y + FOOT_SZ);

  // ── DOWNSCALE 3240×4050 → 1080×1350 for crisp HD output ──
...
// ── Helper: wrap text into lines array ──────────────────────
