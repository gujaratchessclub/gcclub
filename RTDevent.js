...
// ─────────────────────────────────────────────────────────────────────────────
// SHARE CARD — 1080×1350 landscape-friendly, NO links, HD 3x density
// WhatsApp shows this fully without cropping in chat thumbnail
// ─────────────────────────────────────────────────────────────────────────────
async function generatePortraitCard(ev, deepLink) {
  // 1080×1350 = Instagram portrait ratio — WhatsApp shows fully in chat
  // We render at 3× for HD sharpness: 3240×4050, then downscale to 1080×1350
  const SCALE = 3;
  const OUT_W = 1080, OUT_H = 1350;
  const W = OUT_W * SCALE; // 3240
  const H = OUT_H * SCALE; // 4050
  ...
  // All sizes scaled ×3 for HD
  const PAD  = 52 * SCALE;  // horizontal padding
  const TW   = W - PAD * 2; // content width
  let y = 0;
  ...
  // Pick font size: scale down for long names
  const nameSzBase = evName.length > 55 ? 22 : evName.length > 38 ? 26 : 30;
  const nameSz = nameSzBase * SCALE;
  c.font = `800 ${nameSz}px Arial, sans-serif`;
  c.fillStyle = "#1A237E";
  const nameLines = imgWrap(c, evName, TW, nameSz);
  nameLines.forEach(line => { c.fillText(line, PAD, y); y += nameSz + 10 * SCALE; });
  y += 4 * SCALE;

  // ── 2. ORGANIZER ──
  const orgSz = 15 * SCALE;
  c.font = `400 ${orgSz}px Arial, sans-serif`;
  c.fillStyle = "#78909C";
  c.fillText("🏆 " + (ev.organizerName || "—"), PAD, y);
  y += orgSz + 18 * SCALE;

  // divider
  c.strokeStyle = "#E0E0E0"; c.lineWidth = 2 * SCALE;
  c.beginPath(); c.moveTo(PAD, y); c.lineTo(W - PAD, y); c.stroke();
  y += 18 * SCALE;

  // Remaining height for all sections
  const remaining = H - y - 60 * SCALE; // reserve footer space

  // Pre-calculate all section content to measure total needed height
  const fees = ev.entryFees || {};
  const eb = fees.earlyBird || {}, ac = fees.actual || {}, le = fees.lateEntry || {};
  const fRows = [];
  ...
  // Section count for header bars
  const secCount = 3 + (tdRows.length ? 1 : 0); // details + fees + arbiters + optionally TD
  const secHeaderH = 36 * SCALE; // each section header height

  // Estimate total rows height at base row height, then scale down if needed
  // We try rowH values from 52 down to 30 until everything fits
  let rowH = 52 * SCALE;
  let labelSz = 12 * SCALE;
  let valSz   = 15 * SCALE;

  function estimateHeight(rH) {
    // For each row we need to measure value wrapping
    // Quick estimate: assume venue wraps to 2 lines, others 1
    const allRows = detRows.length + fRows.length + aRows.length + tdRows.length;
    const venueExtraLines = Math.ceil((ev.venue||"").length / 40) - 1; // rough extra lines for venue
    return (allRows + venueExtraLines) * rH + secCount * (secHeaderH + 16*SCALE) + 24*SCALE*secCount;
  }

  // Auto-shrink rowH until content fits in remaining space
  while (estimateHeight(rowH) > remaining && rowH > 28 * SCALE) {
    rowH -= SCALE;
    labelSz = Math.max(9 * SCALE, labelSz - 0.3 * SCALE);
    valSz   = Math.max(11 * SCALE, valSz - 0.3 * SCALE);
  }

  // Draw function for a row using dynamic sizes
  function drawRow(label, value, rowIdx, total, tableX, tableW) {
    const LW = Math.floor(tableW * 0.40);
    c.font = `400 ${valSz}px Arial, sans-serif`;
    const vLines = imgWrap(c, String(value || "—"), tableW - LW - 20*SCALE, valSz);
    const rH2 = Math.max(rowH, vLines.length * (valSz + 6*SCALE) + 16*SCALE);
    // bg
    c.fillStyle = rowIdx % 2 === 0 ? "#FFFFFF" : "#F8FBFF";
    c.fillRect(tableX, y, tableW, rH2);
    c.fillStyle = "#EEF4FF";
    c.fillRect(tableX, y, LW, rH2);
    // borders
    if (rowIdx < total - 1) {
      c.strokeStyle = "#DDEEFF"; c.lineWidth = 1.5 * SCALE;
      c.beginPath(); c.moveTo(tableX, y + rH2); c.lineTo(tableX + tableW, y + rH2); c.stroke();
    }
    c.strokeStyle = "#DDEEFF"; c.lineWidth = 1.5 * SCALE;
    c.beginPath(); c.moveTo(tableX + LW, y); c.lineTo(tableX + LW, y + rH2); c.stroke();
    // label
    c.font = `700 ${labelSz}px Arial, sans-serif`;
    c.fillStyle = "#1565C0";
    c.textAlign = "left";
    c.fillText(label.toUpperCase(), tableX + 14*SCALE, y + rH2/2 + labelSz*0.35);
    // value
    c.font = `400 ${valSz}px Arial, sans-serif`;
    c.fillStyle = "#37474F";
    let vy = y + (rH2 - vLines.length * (valSz + 6*SCALE)) / 2 + valSz;
    vLines.forEach(line => { c.fillText(line, tableX + LW + 14*SCALE, vy); vy += valSz + 6*SCALE; });
    y += rH2;
    return rH2;
  }

  // Draw section title
  function drawSec(title) {
    c.fillStyle = "#1565C0";
    c.fillRect(PAD, y, TW, secHeaderH);
    c.font = `700 ${14*SCALE}px Arial, sans-serif`;
    c.fillStyle = "#FFFFFF";
    c.textAlign = "left";
    c.fillText(title, PAD + 16*SCALE, y + secHeaderH*0.68);
    y += secHeaderH;
  }

  // ── 3. EVENT DETAILS ──
  drawSec("📋 Event Details");
  const dtsy = y;
  detRows.forEach(([l,v],i) => drawRow(l, v, i, detRows.length, PAD, TW));
  c.strokeStyle = "#BBDEFB"; c.lineWidth = 2*SCALE; c.strokeRect(PAD, dtsy, TW, y - dtsy);
  y += 18*SCALE;

  // ── 4. ENTRY FEES ──
  drawSec("💰 Entry Fees");
  const fsy = y;
  fRows.forEach(([l,v],i) => drawRow(l, v, i, fRows.length, PAD, TW));
  c.strokeStyle = "#BBDEFB"; c.lineWidth = 2*SCALE; c.strokeRect(PAD, fsy, TW, y - fsy);
  y += 18*SCALE;

  // ── 5. TOURNAMENT DIRECTOR ──
  if (tdRows.length) {
    drawSec("🎯 Tournament Director");
    const tdsy = y;
    tdRows.forEach(([l,v],i) => drawRow(l, v, i, tdRows.length, PAD, TW));
    c.strokeStyle = "#BBDEFB"; c.lineWidth = 2*SCALE; c.strokeRect(PAD, tdsy, TW, y - tdsy);
    y += 18*SCALE;
  }
  ...
  drawSec("⚖️ Arbiters");
  const asy = y;
  aRows.forEach(([l,v],i) => drawRow(l, v, i, aRows.length, PAD, TW));
  c.strokeStyle = "#BBDEFB"; c.lineWidth = 2*SCALE; c.strokeRect(PAD, asy, TW, y - asy);
  y += 18*SCALE;

  // ── 7. FOOTER ──
  // Push footer to bottom of card
  y = H - 52*SCALE;
  c.strokeStyle = "#E0E0E0"; c.lineWidth = 1.5*SCALE;
  c.beginPath(); c.moveTo(PAD, y); c.lineTo(W-PAD, y); c.stroke();
  y += 16*SCALE;
  c.font = `400 ${12*SCALE}px Arial, sans-serif`;
  c.fillStyle = "#90A4AE";
  c.textAlign = "center";
  c.fillText("Rated Events GJ  •  Gujarat Chess Club", W/2, y + 22*SCALE);

  // ── OUTPUT: downscale 3240×4050 → 1080×1350 for final HD JPG ──
  ...
  return fin.toDataURL("image/jpeg", 1.0); // max quality
}

// Section title — popup-section-title style
function imgSecTitle(c,title,x,y,w){
  c.font="700 24px Arial, sans-serif";
  c.fillStyle="#1565C0";
  c.textAlign="left";
  c.fillText(title,x,y+28);
  c.strokeStyle="#BBDEFB";c.lineWidth=3;
  c.beginPath();c.moveTo(x,y+40);c.lineTo(x+w,y+40);c.stroke();
  return y+58;
}

// Table row — popup-detail-table style
function imgRow(c,label,value,x,y,w,idx,total){
  const LW=Math.floor(w*0.40);
  const VW=w-LW;
  c.font="400 26px Arial, sans-serif";
  const vLines=imgWrap(c,String(value||"—"),VW-32,26);
  const rH=Math.max(64,vLines.length*34+28);
  // row bg
  c.fillStyle=idx%2===0?"#FFFFFF":"#F8FBFF";
  c.fillRect(x,y,w,rH);
  // label cell bg
  c.fillStyle="#EEF4FF";
  c.fillRect(x,y,LW,rH);
  // dividers
  if(idx<total-1){c.strokeStyle="#DDEEFF";c.lineWidth=1.5;c.beginPath();c.moveTo(x,y+rH);c.lineTo(x+w,y+rH);c.stroke();}
  c.strokeStyle="#DDEEFF";c.lineWidth=1.5;
  c.beginPath();c.moveTo(x+LW,y);c.lineTo(x+LW,y+rH);c.stroke();
  // label
  c.font="700 21px Arial, sans-serif";
  c.fillStyle="#1565C0";
  c.textAlign="left";
  c.fillText(label.toUpperCase(),x+16,y+rH/2+8);
  // value
  c.font="400 26px Arial, sans-serif";
  c.fillStyle="#37474F";
  let vy=y+(rH-vLines.length*34)/2+26;
  vLines.forEach(line=>{c.fillText(line,x+LW+16,vy);vy+=34;});
  return y+rH;
}

// Wrap text — returns array of lines
function imgWrap(c,text,maxW,sz){
...
// roundRect
function imgRound(ctx,x,y,w,h,r){
  ctx.beginPath();
  ctx.moveTo(x+r,y);
